...
1. **Question bank:** this repository contains 40 starter questions. Expand only with clinically reviewed, licensed/original content.
2. **Tutor:** Ada is currently an offline knowledge-base tutor. It does not call a remote LLM.
3. **Payments:** the pricing screen is a product-flow demo. It does not charge users. Before release, integrate Apple App Store / Google Play billing and server-side entitlement validation.
4. **Accounts/cloud sync:** progress is stored on-device only. Add authentication and a backend before multi-device release.
5. **Medical review:** all educational clinical content should be reviewed by qualified nursing educators before publication.
6. **Readiness:** the readiness score is a NursePass study metric, not an official NCLEX pass prediction.
